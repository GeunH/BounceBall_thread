...
int i = 0;
HANDLE hThread;
LPCTSTR lpszClass1 = TEXT("Bounce2");
...
{
    if (btnflag == 0) {
        MessageBox(hWnd, TEXT("2번 프로그램이 실행되었습니다."), TEXT("OK"), MB_OK);
...
void moveBall()
{
    ...
    if (x <= -R && turn_flag == 1)
    {
        ...
        HWND* hWMd = FindWindow(NULL, L"Bounce1");
    ...
    else if (x >= crt.right + R  && turn_flag == 1)
    {
        COPYDATASTRUCT pcds;
        data Data = { 1,y+yv, xv, yv };
        pcds.dwData = &Data;
        pcds.cbData = sizeof(Data);
        pcds.lpData = (LPVOID)&Data;
        HWND* hWMd = FindWindow(NULL, L"Bounce3");
        SendMessage(hWMd, WM_COPYDATA, NULL, (LPARAM)&pcds);
        turn_flag = 0;
